...
export function PublicOffRegistration() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (Date.now() - lastSubmittedAt < 30_000) {
      setError(
        "Bạn vừa gửi yêu cầu này gần đây. Vui lòng chờ 30 giây trước khi gửi lại.",
      );
      setSubmitting(false);
      return;
    }
  ...
  return (
    <main className="off-public-page">
      <header className="off-public-header">
        <a
          href="/off-registration"
          className="off-public-brand"
          aria-label="Rider Operations"
        >
          <span>
            <CalendarCheck2 size={20} aria-hidden="true" />
          </span>
          <div>
            <strong>Rider Operations</strong>
            <small>KV5 + KV6 · SDD</small>
          </div>
        </a>
        <div className="off-public-secure">
      ...
      <section className="off-public-layout">
        <aside className="off-public-brief">
          ...
          <p>
            Gửi ngày nghỉ để điều phối viên kiểm tra và xếp lịch. Yêu cầu chỉ có
            hiệu lực sau khi được duyệt.
          </p>
          <dl>
            <div>
              <dt>01</dt>
              <dd>
                <strong>Xác minh rider</strong>
                <span>Mã rider và họ tên đầy đủ.</span>
              </dd>
            </div>
            <div>
              <dt>02</dt>
              <dd>
                <strong>Chọn nhiều ngày</strong>
                <span>Một đơn OFF phép có thể gồm nhiều ngày.</span>
              </dd>
            </div>
            <div>
              <dt>03</dt>
              <dd>
                ...
                <span>Email báo ngay khi yêu cầu được duyệt hoặc từ chối.</span>
              </dd>
            </div>
          </dl>
          <div className="off-public-note">
            <ShieldCheck size={18} aria-hidden="true" />
            <span>
              Trang này không hiển thị dữ liệu hay lịch của rider khác.
            </span>
          </div>
        </aside>
        ...
        <div className="off-public-workbench">
          {receipt ? (
            <section className="off-public-receipt" aria-live="polite">
              <span className="off-public-receipt-mark">
                <Check size={28} aria-hidden="true" />
              </span>
              <p>Đã ghi nhận yêu cầu</p>
              <h2>{receipt.name || form.rider_code}</h2>
              <div>
                <span>Số ngày OFF</span>
                <strong>{receipt.dates.length} ngày</strong>
              </div>
              <div>
                <span>Các ngày đã gửi</span>
                <strong>
                  {receipt.dates
                    .map((date) =>
              ...
              <div>
                <span>Mã đơn</span>
                <strong>{receipt.batchId.slice(0, 8).toUpperCase()}</strong>
              </div>
              <small>
                Trạng thái hiện tại: Chờ duyệt. Đây chưa phải xác nhận nghỉ
                chính thức.
              </small>
              <button
                type="button"
            ...
            <form className="off-public-form" onSubmit={submit}>
              ...
              <div className="off-public-fields is-two">
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      update("rider_code", e.target.value.toUpperCase())
                    }
                    placeholder="VD: 196"
                  />
                </label>
              ...
              <div className="off-public-fields is-email">
                <label>
                  <span>Email nhận kết quả</span>
                  <div className="off-public-email-input">
                  ...
                  <small>
                    Hệ thống sẽ gửi mail khi yêu cầu được duyệt hoặc từ chối.
                  </small>
                </label>
              </div>
              <section
                className="off-public-calendar"
                aria-label="Chọn nhiều ngày OFF"
              >
                <header>
                  <div>
                    <span>Ngày OFF phép</span>
                    <strong>{selectedDates.length} ngày đã chọn</strong>
                  </div>
                  <div>
                    <button
                      type="button"
                      ...
                      onClick={() =>
                        setCalendarMonth((date) => subMonths(date, 1))
                      }
                    >
                      <ChevronLeft size={16} />
                    </button>
                    ...
                    <button
                      type="button"
                      disabled={
                        format(calendarMonth, "yyyy-MM") >= maxDate.slice(0, 7)
                      }
                ...
                <div className="off-public-calendar-weekdays">
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      update(
                        "request_type",
                        e.target.value as FormState["request_type"],
                      )
                    }
                  >
                    <option value="PLANNED">OFF phép</option>
                    <option value="WEEKLY">OFF tuần</option>
                    <option value="EMERGENCY">OFF đột xuất</option>
                  </select>
                  <small>Áp dụng cho toàn bộ ngày đã chọn</small>
                </label>
              </div>
              <input
                aria-hidden="true"
              ...
              />
              <label className="off-public-reason">
                <span>
                  Lý do / ghi chú <small>không bắt buộc</small>
                </span>
                <textarea
                  rows={4}
                  maxLength={500}
                  ...
                  placeholder="Thông tin giúp điều phối viên xét duyệt..."
                />
              </label>
              <section className="off-public-evidence">
                ...
                />
                {evidencePreview ? (
                  <div className="off-public-evidence-preview">
                    <NextImage
                      unoptimized
                      width={64}
                      height={64}
                  ...
                  <button
                    className="off-public-evidence-picker"
                    type="button"
                    onClick={() => evidenceInputRef.current?.click()}
                  >
                    <ImagePlus size={19} aria-hidden="true" />
                    <span>
                      <strong>Thêm ảnh bằng chứng</strong>
                      <small>JPG, PNG hoặc WebP · tối đa 5 MB</small>
                    </span>
                  </button>
                )}
              </section>
              <p className="off-public-helper">
                Yêu cầu trùng ngày sẽ không được tạo thêm. Ảnh bằng chứng là tùy
                chọn.
              </p>
              {error ? (
                <p className="off-public-error" role="alert">
                  {error}
                </p>
              ) : null}
              <button
                className="off-public-submit"
                type="submit"
                disabled={submitting}
              >
                {submitting ? "Đang gửi..." : "Gửi yêu cầu OFF"}
              </button>
            </form>
          )}
        </div>
      </section>
      <footer className="off-public-footer">
        <span>SDD Rider Operations</span>
        <span>Yêu cầu được lưu để phục vụ xếp lịch nội bộ.</span>
      </footer>
    </main>
  );
}
